...
async def command(message: types.Message) -> None:
    try:
        await bot.send_message(message.from_user.id, text="Важно ли для вас наличие море/океана в новой стране",
...
async def water_answer(callback: types.CallbackQuery, callback_data: dict):
    await callback.message.edit_text(
        text='Вы хотите жить где скорость жизни быстрая или где скорость жизни маленькая и нет суеты',
...
async def family_answer(callback: types.CallbackQuery, callback_data: dict):
    await callback.message.edit_text(
        text='Вы планируете передвигаться на общественом транспорте или на собственом автомобиле',
...
async def flat_answer(callback: types.CallbackQuery, callback_data: dict):
    if answer_user['family'] == "1":
        await callback.message.edit_text(
            text='Выберете заработок на который вы расчитаваете (если мы не сможем найти в нужном диапазоне мы найдет ближайший подходящий)',
